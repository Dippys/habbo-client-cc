...
foreach ($paths as $path) {
    ...
    if (strpos($data, 'extends BitmapAsset') === false)
        continue;
    ...
    $tmp .= $path.": ".$pathDir.'/'.$sourceName.": ". $pathDir.'/'.$nameTemplate.".png\n";

    $deleteFile[] = $pathDir.'/'.$sourceName;
    copy($pathDir.'/'.$sourceName, $pathDir.'/'.$nameTemplate.".png");

    $data = str_replace('source="'.$sourceName, 'source="'.$nameTemplate.".png", $data);
    file_put_contents($path, $data);
}
asort($doublons);

file_put_contents(__DIR__ . "/../tmp2.txt", print_r($doublons, true));

foreach($deleteFile as $file) {
    unlink($file);
}

file_put_contents(__DIR__ . "/../tmp.txt", $tmp);
